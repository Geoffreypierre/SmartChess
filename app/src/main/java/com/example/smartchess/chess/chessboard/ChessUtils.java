...
public class ChessUtils {
    ...
    public static boolean isKingInCheck(Piece[][] board, Piece.Color color, Move enPassantSquare) {
        ...
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                ...
                if (piece != null && piece.getColor() != color) {
                    List<Move> moves = piece.getAvailableMoves(row, col, board,enPassantSquare);
                    for (Move move : moves) {
    ...
    public static boolean doesMoveResolveCheck(Piece[][] board, Piece.Color color, int fromRow, int fromCol, Move move, Move enPassantSquare) {
    ...
    public static boolean CanAPieceMove(Piece[][] board, Piece.Color color, Move enPassantSquare) {
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                Piece piece = board[row][col];
                if (piece != null && piece.getColor() == color) {
                    List<Move> moves = piece.getAvailableMovesWithCheck(row, col, board,enPassantSquare);
                    if (!moves.isEmpty()) {
    ...
    public static boolean isSquareUnderAttack(Piece[][] board, int targetRow, int targetCol, Piece.Color defenderColor, Move enPassantSquare) {
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                Piece piece = board[row][col];
